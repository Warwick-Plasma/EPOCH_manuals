...
\begin{boxverbatim}
$> idl Start
IDL Version 7.1.1, Mac OS X (darwin x86_64 m64). (c) 2009, ITT Visual Informatio
n Solutions
Installation number: .
...
1) electron (Point_grid) : 2D Point mesh
2) proton (Point_grid) : 2D Point mesh
...
8) electron_Py (Particles) : 1D Point variable
10) electron_Pz (Particles) : 1D Point variable
11) proton_Pz (Particles) : 1D Point variable
12) Ex (Electric Field) : 2D Plain variable
13) Ey (Electric Field) : 2D Plain variable
14) Ez (Electric Field) : 2D Plain variable
15) Bx (Magnetic Field) : 2D Plain variable
16) By (Magnetic Field) : 2D Plain variable
17) Bz (Magnetic Field) : 2D Plain variable
18) Jx (Current) : 2D Plain variable
19) Jy (Current) : 2D Plain variable
20) Jz (Current) : 2D Plain variable
21) en_electron (Grid) : 1D Plain mesh
22) en_electron (Norm_Grid) : 1D Plain mesh
23) en_electron (dist_fn) : 1D Plain variable
24) electron_probe (Probe_Grid) : 2D Point mesh
25) Px (electron_probe) : 1D Point variable
26) Py (electron_probe) : 1D Point variable
27) Pz (electron_probe) : 1D Point variable
28) weight (electron_probe) : 1D Point variable
IDL>
\end{boxverbatim}
...
\begin{boxverbatim}
IDL> gridclass = getdata(2,/grid)

IDL> help,gridclass,/structures
** Structure <6008008>, 10 tags, length=67688, data length=67680, refs=1:
   FILENAME        STRING    'Data/0002.sdf'
   SNAPSHOT        LONG                47
   TIME            DOUBLE       6.0440087e-14
   PARTICLES_ELECTRON
   ...
   GRID_EN_ELECTRON
                   STRUCT    -> <Anonymous> Array[1]
   NORM_GRID_EN_ELECTRON
                   STRUCT    -> <Anonymous> Array[1]
   GRID_EN_PROTON  STRUCT    -> <Anonymous> Array[1]
   NORM_GRID_EN_PROTON
                   STRUCT    -> <Anonymous> Array[1]

IDL> help,gridclass.grid,/structures
** Structure <5301a18>, 6 tags, length=344, data length=340, refs=2:
   X               DOUBLE    Array[20]
   Y               DOUBLE    Array[20]
   MESHTYPE        LONG                 0
   ND              LONG                 2
...
IDL>
\end{boxverbatim}
  \begin{center}\includegraphics[width=0.8\linewidth]{images/idl_help}\end{center}
...
\end{boxverbatim}
  ...
  \begin{center}\includegraphics[width=0.8\linewidth]{images/idl_simple_plot}\end{center}

  As a more concrete example, we will now take a one-dimensional slice through
  the 2D \qtt{Number\_Density} array read in from our SDF data file. In this
  example we will give the $x$ and $y$ axes labels by passing extra parameters
  to the \qtt{plot} routine. A full list of parameters can be found in the
  on-line documentation. In this example we also make use of the \qtt{\$} 
  symbol which is IDL's line continuation character.

\begin{boxverbatim}
IDL> data = getdata(0)
IDL> plot,data.grid.x,data.number_density[*,50],xtitle='x', $
IDL>    ytitle='number density'
\end{boxverbatim}
  This command generates the following plot:
  \begin{center}\includegraphics[width=0.8\linewidth]{images/idl_plot}\end{center}
...
\end{boxverbatim}
  ...
  \begin{center}\includegraphics[width=0.8\linewidth]{images/idl_ps_plot}\end{center}
...
\begin{boxverbatim}
IDL> contour,data.number_density,data.grid.x,data.grid.y,xstyle=1, $
IDL>    ystyle=1,nlevels=200,/fill
\end{boxverbatim}
  ...
  \begin{center}\includegraphics[width=0.8\linewidth]{images/idl_contour}\end{center}
...
\begin{boxverbatim}
IDL> loadct,3
IDL> shade_surf,data.number_density,data.grid.x,data.grid.y,xstyle=1, $
IDL>    ystyle=1,xtitle='x',ytitle='y',ztitle='number density',charsize=3
\end{boxverbatim}
  \begin{center}\includegraphics[width=0.8\linewidth]{images/idl_shade_surf}\end{center}
...
\begin{boxverbatim}
IDL> iplot,data.grid.x,data.number_density[*,50]
\end{boxverbatim}
  \begin{center}\includegraphics[width=0.8\linewidth]{images/idl_iplot}\end{center}
...
MatLab software.
  ...
  \item
    Available for UNIX (Irix, Tru64, AIX, Linux, Solaris), Mac OS X (10.3, 10.4), and Windows platforms
  \item
...
\subsection{Obtaining And Installing VisIt}
  ...
  {\tt https://wci.llnl.gov/codes/visit/}

  \begin{center}\includegraphics[width=0.8\linewidth]{images/visit_web}\end{center}
  ...
  \cmd{tar xzf visit1\_12\_2.linux-x86\_64.tar.gz}. The actual name of
...
\subsection{Loading Data Into VisIt}
  \begin{center}\includegraphics[height=0.7\textheight]{images/visit_db_list}\end{center}
  ...
  Select file} from the VisIt menu bar. A file selection dialogue will appear
  ...
  \begin{center}\includegraphics[height=0.7\textheight]{images/visit_select_var}\end{center}
  Once a SDF file has been successfully loaded the \qm{Plots} menu item
...
\subsection{Contour Plots in VisIt}
  ...
  now select the \qm{Plots} menu item. Notice that many of the plot
  ...
  For the current dataset there is no \qm{Curve} plot available since this
  requires a one dimensional scalar array and none of our variables meet
  that criteria. There is also no \qm{Streamline} or \qm{Vector} plots
  since these both act on vector fields.
  ...
  choose \qm{Plots $\Rightarrow$ Pseudocolor $\Rightarrow$ Derived $\Rightarrow$
  Number\_Density} and then hit the \qm{Draw} button.
  \begin{center}\includegraphics[width=0.8\linewidth]{images/visit_contour}\end{center}

  There are many, many settings which can alter the visual appearance
  of plots generated by VisIt. The first point of call is usually to
  open up the \qm{PlotAtts} or \qm{OpAtts} dialogue corresponding to the
  ...
  launch the corresponding \qm{PlotAtts} dialogue.
  ...
  will launch the corresponding \qm{OpAtts} dialogue.
  \begin{center}\includegraphics[width=0.7\linewidth]{images/visit_attrib}\end{center}
  ...
  \begin{center}\includegraphics[width=0.6\linewidth]{images/visit_annot}\end{center}
...
\subsection{1D Plotting in VisIt}
  ...
  selected.
  \begin{center}\includegraphics[width=0.8\linewidth]{images/visit_curve}\end{center}
...
\subsection{Shaded Surface Plots in VisIt}
  ...
  up.
  \begin{center}\includegraphics[width=0.8\linewidth]{images/visit_shade_surf}\end{center}
...
\subsection{Creating User-Defined Expressions}
  ...
  \begin{center}\includegraphics[width=0.8\linewidth]{images/visit_expression_vector}\end{center}

  Now when we return to the \qm{Plots} menu we see that the \qm{Vector}
  and \qm{Streamline} plot types have been activated and we can use them
  to plot our newly defined vector.
...
\subsection{Creating Movies}
  ...
  \begin{center}\includegraphics[width=0.8\linewidth]{images/visit_movie}\end{center}
...
\subsection{Remote Visualisation}
  ...
  \begin{center}\includegraphics[width=0.8\linewidth]{images/visit_host_profile}\end{center}
  ...
  $\Rightarrow$ Select file} and picking the desired host profile
  ...
  \begin{center}\includegraphics{images/visit_host_files}\end{center}
